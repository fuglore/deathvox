...
function CopActionTase:on_attention(attention)
	...
	self._mod_enable_t = t + 0.2 --This is normally bound to shoot delay, but really, the taser only has a .1 aim delay, so it shouldn't matter, increased slightly from that to match turn animation.
	self._tasing_local_unit = nil
	self._tasing_player = nil
